...
def load_zealy_index() -> None:
    global ZEALY_INDEX
    ZEALY_INDEX = {}
    csv_path = _discover_latest_zealy_csv()
    if not csv_path.exists():
        log.warning("Zealy CSV not found at %s", csv_path)
        return
    try:
        ...
        log.error("Failed to load Zealy CSV: %s", e)
...
async def admin_upload_zealy_csv(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        load_zealy_index()
        total = len(ZEALY_INDEX)
        await update.message.reply_text(f"✅ CSV imported. Indexed users: {total}")
    except Exception as e:
...
async def admin_import_list_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not update.message or not update.message.reply_to_message:
        await update.message.reply_text("Reply to a CSV document with this command.")
        return
    replied = update.message.reply_to_message
    if not replied.document:
        await update.message.reply_text("The replied message does not contain a document.")
        return
    ...
    try:
        load_zealy_index()
        total = len(ZEALY_INDEX)
        await update.message.reply_text(f"✅ CSV imported from reply. Indexed users: {total}")
    except Exception as e:
